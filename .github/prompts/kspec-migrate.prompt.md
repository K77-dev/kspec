...
O argumento recebido é: **$PROMPT**

Se `$PROMPT` estiver vazio, peça ao usuário para informar a dependência e versão alvo (ex: `/kspec-migrate react@19`) e não prossiga até receber.
...
Executar todos os checks conforme definido em "Comandos do projeto" no copilot-instructions.md (lint, typecheck, build, test).
