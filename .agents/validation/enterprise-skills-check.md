...
1. Check if the cache directory `.agents/.enterprise-skills-cache/` exists.
2. If the cache directory exists and contains a `.git/` subdirectory:
   - Run: `git -C .agents/.enterprise-skills-cache/ pull --ff-only`
3. If the cache directory does NOT exist:
   - Run: `git clone --depth 1 --branch {branch} {url} .agents/.enterprise-skills-cache/`
...
1. Read `.agents/.enterprise-skills-cache/skills-lock.json`.
...
1. Copy the skill directory from the cache:
   ```
   cp -r .agents/.enterprise-skills-cache/.agents/skills/{skill-name}/ .agents/skills/{skill-name}/
   ```
2. Create the symlink in `.agents/skills/`:
   ```
...
- The cache directory `.agents/.enterprise-skills-cache/` is in `.gitignore` and should NOT be committed.
